...
if(isset($_GET['kw'])) {
    ...
    $numrows=$DB->count("SELECT count(*) from yixi_adver WHERE see=0{$sql}");
    $con='包含 '.$kw.' 的共有 <b>'.$numrows.'</b> 个用户广告待审核';
    $link='&kw='.$_GET['kw'];
}else{
    $numrows=$DB->count("SELECT count(*) from yixi_adver WHERE see=0");
    $con='平台共有 <b>'.$numrows.'</b> 个用户广告待审核';
}
?>
     <div style="white-space:nowrap;overflow-x: auto;">
        <table class="layui-table layuiadmin-page-table">
          <thead><tr><th>广告ID</th><th>广告标题</th><th>跳转地址</th><th>文字颜色</th><th>管理用户</th><th>广告图标</th><th>购买时间</th><th>到期时间</th><th>是否激活</th><th>操作</th></tr></thead>
          <tbody>
<?php
...
$rs=$DB->query("SELECT * FROM yixi_adver WHERE see=0{$sql} order by id asc limit $offset,$pagesize");
...
echo '<tr><td><b>'.$res['id'].'</b></td><td>'.substr($res['title'],0,20).'</td><td><a target="_blank" href="'.$res['url'].'"><i class="fa fa-internet-explorer"></i>'.substr($res['url'],0,20).'</a></td><td><span style="color:'.$res['colour'].';">'.$res['colour'].'</span></td><td>'.$daili.'</td><td>'.$icon.'</td><td>'.$res['date'].'</td><td>'.$res['last'].'</td><td><input type="checkbox" id="switch'.$res['id'].'" onclick="Active('.$res['id'].')"'.($res['active']==1?' checked ':' ').'data-switch="success"/><label for="switch'.$res['id'].'" data-on-label="是" data-off-label="否" class="mb-0 d-block"></label></td><td><span class="layui-btn layui-btn-xs btn-success" onclick="see_tg('.$res['id'].')">通过</span><span class="layui-btn layui-btn-xs btn-danger" onclick="see_btg('.$res['id'].')">不通过</span></td></tr>';
}
?>
          </tbody>
        </table>
