    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                验证码发送日志&nbsp;&nbsp;&nbsp;<a href="javascript:searchClear()" class="badge badge-danger-info" title="刷新广告列表"><i class="layui-icon layui-icon-refresh"></i> 刷新</a>
            </div>
            <div class="card-body">
                <div class="layui-elem-quote" id="blocktitle"></div>
                <form onsubmit="return searchOrder()" method="GET" class="form layui-form">
                    <div class="form-group mb-3">
                        <select class="form-control" name="type" lay-search><option value="0">全部</option><option value="1">接收号码</option><option value="2">验证码</option><option value="3">IP</option></select>
                    </div>
...
function listTable(query){
    ...
    if(query == 'start' || query == undefined){
        query = '';
        history.replaceState({}, null, './codelogs.php');
    }else if(query != undefined){
        history.replaceState({}, null, './codelogs.php?'+query);
    }
    layer.closeAll();
    var ii = layer.msg('正在获取验证码日志中,请稍后...', {icon: 16, time: 10 * 1000});
    $.ajax({
        type : 'GET',
        url : 'codelogs-table.php?'+query,
        dataType : 'html',
        cache : false,
